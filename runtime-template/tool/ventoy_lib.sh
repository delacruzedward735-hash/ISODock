...
vtinfo() {
    if [ -t 1 ]; then echo -e "\033[32m$*\033[0m"; else echo "$*"; fi
}

vtwarn() {
    if [ -t 1 ]; then echo -e "\033[33m$*\033[0m"; else echo "$*"; fi
}


vterr() {
    if [ -t 1 ]; then echo -e "\033[31m$*\033[0m"; else echo "$*"; fi
}
...
check_umount_disk() {
    ...
    # Desktop Linux commonly automounts USB partitions.  Unmount by source
    # block device instead of the mountpoint string from /proc/mounts because
    # mountpoint spaces are escaped there as \\040 (for example "ZORIN OS").
    sync

    if command -v lsblk >/dev/null 2>&1 && command -v findmnt >/dev/null 2>&1; then
        # Children first, then the whole device.  tac is provided by coreutils;
        # fall back to the natural order if it is unavailable.
        if command -v tac >/dev/null 2>&1; then
            VTOY_NODES="$(lsblk -nrpo NAME "$DiskOrPart" 2>/dev/null | tac)"
        else
            VTOY_NODES="$(lsblk -nrpo NAME "$DiskOrPart" 2>/dev/null)"
        fi

        echo "$VTOY_NODES" | while IFS= read -r srcdev; do
            [ -n "$srcdev" ] || continue
            if findmnt -rn -S "$srcdev" >/dev/null 2>&1; then
                vtdebug "Trying to umount source $srcdev ..."
                umount --all-targets -- "$srcdev" >/dev/null 2>&1 || \
                    umount -- "$srcdev" >/dev/null 2>&1 || true
            fi
        done
        ...
        # A desktop automounter may race the first unmount. Retry once.
        echo "$VTOY_NODES" | while IFS= read -r srcdev; do
            [ -n "$srcdev" ] || continue
            if findmnt -rn -S "$srcdev" >/dev/null 2>&1; then
                vtdebug "Retry umount source $srcdev ..."
                umount --all-targets -- "$srcdev" >/dev/null 2>&1 || \
                    umount -- "$srcdev" >/dev/null 2>&1 || true
            fi
        done
    else
        grep "^${DiskOrPart}" /proc/mounts | awk '{print $1}' | sort -u | while IFS= read -r srcdev; do
            [ -n "$srcdev" ] || continue
            vtdebug "Trying to umount source $srcdev ..."
            umount -- "$srcdev" >/dev/null 2>&1 || true
        done
